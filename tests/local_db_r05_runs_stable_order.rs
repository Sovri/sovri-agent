...
//! R-05 -- runs can be retrieved by run id in stable order. Covers issue #348.
...
impl TempDatabase {
    fn new() -> Self {
        ...
        let root = std::env::temp_dir().join(format!(
            "sovri-agent-mat98-r05-runs-stable-order-{}-{now}-{unique}",
            std::process::id()
        ));
        TempDatabase {
...
#[allow(dead_code)]
trait LocalDatabaseRunQueries {
    fn query_runs(&self) -> Result<Vec<String>, LocalDatabaseError> {
        panic!("LocalDatabase::query_runs is not implemented")
    }
}

impl LocalDatabaseRunQueries for LocalDatabase {}

#[test]
fn runs_can_be_retrieved_by_run_id_in_stable_order() {
    ...
    // When the operator queries runs.
    let first_query = local_database
        .query_runs()
        .expect("the run list can be queried");

    // Then the run list includes "classified-evidence-2026-06-24" before "mixed-2026-06-24".
    assert_run_precedes(&first_query, CLASSIFIED_RUN_ID, MIXED_RUN_ID);

    // And querying runs a second time returns the same order.
    let second_query = local_database
        .query_runs()
        .expect("the run list can be queried again");
...
fn assert_run_precedes(run_ids: &[String], earlier: &str, later: &str) {
    let earlier_position = run_ids
        .iter()
        .position(|run_id| run_id == earlier)
        .expect("the earlier run id is returned");
    let later_position = run_ids
        .iter()
        .position(|run_id| run_id == later)
        .expect("the later run id is returned");
    assert!(
        earlier_position < later_position,
        "{earlier} appears before {later}"
    );
}
